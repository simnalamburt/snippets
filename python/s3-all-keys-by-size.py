...
def all_keys(bucket, prefix, regex):
    ...
    '''
    if find_spec('boto3'):
        generator = all_keys_boto3(bucket, prefix, regex)
    elif find_spec('boto'):
        generator = all_keys_boto2(bucket, prefix, regex)
    else:
        raise ModuleNotFoundError("Module 'boto3' or 'boto' is required")
    return [x[0] for x in sorted(generator, key=lambda x: x[1], reverse=True)]
...
def all_keys_boto3(bucket, prefix, regex):
    ...
    while True:
        ...
        yield from ((obj['Key'], obj['Size']) for obj in resp['Contents'] if program.match(obj['Key']))
